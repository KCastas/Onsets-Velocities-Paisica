...
if __name__ == "__main__":
    ...
    def model_inference(x):
        ...
        have same length. --- Modified for a causal, onset-only model.
        """
        probs = model(x, trainable_onsets=False)
        probs = F.pad(torch.sigmoid(probs), (1, 0))
        # Mock velocities as zeros to keep the evaluator from crashing
        vels = torch.zeros_like(probs)
        return probs, vels
    ...
    for epoch in range(1, CONF.NUM_EPOCHS + 1):
        for i, (logmels, rolls, metas) in enumerate(train_dl):
            ...
            with torch.no_grad():
                ...
                # ##############################################################
                # Removed future-frame stretching for causal training
            ...
            # The modified model only returns a single onset tensor
            onset_pred = model(logmels, CONF.TRAINABLE_ONSETS)

            # Calculate only using the single Binary Cross-Entropy loss
            ons_loss = ons_loss_fn(onset_pred, onsets_clip)
            loss = ons_loss
