...
class OnsetsAndVelocities(torch.nn.Module):
    ...
    def get_cam_stage(in_chans, out_bins, conv1x1head=(200, 200),
        ...
        cam = torch.nn.Sequential(
            ...
            *[torch.nn.Sequential(
            ...
            torch.nn.ZeroPad2d((summary_width - 1, 0, 0, 0)), # Causal pad
            torch.nn.Conv2d(
    ...
    def __init__(self, in_chans, in_height, out_height, conv1x1head=(200, 200),
        ...
        self.stem = torch.nn.Sequential(
            ...
            torch.nn.ZeroPad2d((2, 0, 1, 1)), # Causal pad (left=2, right=0)
        ...
        if trainable_onsets:
            ...
            with torch.no_grad():
                ...
                stem_out = torch.cat([stem_out, x_stages[-1].unsqueeze(1)],
                                     dim=1)
        #
        # We only need the first (and only) onset stage for instant feedback
        return x_stages[0]